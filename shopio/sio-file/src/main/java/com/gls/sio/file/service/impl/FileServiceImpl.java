...
public class FileServiceImpl implements FileService {
	...
	public FileEntity storeFile(MultipartFile file) {
		...
		try {
			...
			String fileDownloadUri = ServletUriComponentsBuilder.fromCurrentContextPath().path(environment.getProperty(DOWN_LOAD_URI))
					.path(fileEntity.getFilename()).toUriString();

			fileEntity.setFileDownloadUri(fileDownloadUri);
			
			File image = new File(String.format(environment.getProperty(PRODUCT_IMAGES_DIRECTORY), fileEntity.getFilename()));
